...
select  
sum((select count(likes.user_id) from likes where likes.user_id = profiles.user_id and target_type_id = 2)) as likes_num 
from profiles group by user_id order by birthday desc limit 10;
...
-- активность считаю, как всю активность / время пользования соцсетью (дата создания пользователя - текущая дата, временем создания пренебрегаю)
